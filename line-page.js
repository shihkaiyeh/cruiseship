(function () {
  ...
  const company = catalog.getCompany(routes.pathValue('cruise-lines'));
  ...
  document.getElementById('addLineReview').href = routes.addReviewUrl(company);
  ...
  window.ReviewsUI.loadReviews(
    opinion => catalog.normalize(opinion.line) === catalog.normalize(company.formValue),
    {
      ...
      ctaHref: routes.addReviewUrl(company),
      ctaText: '撰寫第一則評價'
    }
  );
})();
